...
CREATE TABLE IF NOT EXISTS llx_seup_obavijesti (
...
CREATE INDEX IF NOT EXISTS idx_obavijesti_aktivan ON llx_seup_obavijesti(aktivan);
CREATE INDEX IF NOT EXISTS idx_obavijesti_datum ON llx_seup_obavijesti(datum_kreiranja);
CREATE INDEX IF NOT EXISTS idx_obavijesti_tip ON llx_seup_obavijesti(tip);

-- Tablica za praćenje pročitanih obavijesti
CREATE TABLE IF NOT EXISTS llx_seup_obavijesti_procitane (
  rowid bigserial PRIMARY KEY,
  ...
  CONSTRAINT fk_obavijest_procitana FOREIGN KEY (fk_obavijest)
    REFERENCES llx_seup_obavijesti(rowid) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_procitane_user ON llx_seup_obavijesti_procitane(fk_user);
CREATE INDEX IF NOT EXISTS idx_procitane_obavijest ON llx_seup_obavijesti_procitane(fk_obavijest);

-- Enable Row Level Security
ALTER TABLE llx_seup_obavijesti ENABLE ROW LEVEL SECURITY;
ALTER TABLE llx_seup_obavijesti_procitane ENABLE ROW LEVEL SECURITY;

-- Policies za obavijesti
CREATE POLICY "Authenticated users can view active notifications"
  ON llx_seup_obavijesti FOR SELECT
  TO authenticated
  USING (aktivan = true);

CREATE POLICY "Admin users can insert notifications"
  ON llx_seup_obavijesti FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Admin users can update notifications"
  ON llx_seup_obavijesti FOR UPDATE
  TO authenticated
  USING (auth.uid() IS NOT NULL)
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Admin users can delete notifications"
  ON llx_seup_obavijesti FOR DELETE
  TO authenticated
  USING (auth.uid() IS NOT NULL);

-- Policies za pročitane obavijesti
CREATE POLICY "Users can view their own read notifications"
  ON llx_seup_obavijesti_procitane FOR SELECT
  TO authenticated
  USING (fk_user = (auth.jwt() ->> 'sub')::integer);

CREATE POLICY "Users can mark notifications as read"
  ON llx_seup_obavijesti_procitane FOR INSERT
  TO authenticated
  WITH CHECK (fk_user = (auth.jwt() ->> 'sub')::integer);

CREATE POLICY "Users can delete their own read marks"
  ON llx_seup_obavijesti_procitane FOR DELETE
  TO authenticated
  USING (fk_user = (auth.jwt() ->> 'sub')::integer);
