/*
  # SEUP Obavijesti Modul (bez fiksnog prefixa)
...
CREATE TABLE IF NOT EXISTS seup_obavijesti (
...
CREATE INDEX IF NOT EXISTS idx_obavijesti_aktivan ON seup_obavijesti(aktivan);
CREATE INDEX IF NOT EXISTS idx_obavijesti_datum ON seup_obavijesti(datum_kreiranja);
CREATE INDEX IF NOT EXISTS idx_obavijesti_tip ON seup_obavijesti(tip);

-- Tablica za praćenje pročitanih obavijesti
CREATE TABLE IF NOT EXISTS seup_obavijesti_procitane (
  rowid bigserial PRIMARY KEY,
  ...
  CONSTRAINT fk_obavijest_procitana FOREIGN KEY (fk_obavijest)
    REFERENCES seup_obavijesti(rowid) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_procitane_user ON seup_obavijesti_procitane(fk_user);
CREATE INDEX IF NOT EXISTS idx_procitane_obavijest ON seup_obavijesti_procitane(fk_obavijest);

-- Enable Row Level Security
ALTER TABLE seup_obavijesti ENABLE ROW LEVEL SECURITY;
ALTER TABLE seup_obavijesti_procitane ENABLE ROW LEVEL SECURITY;

-- Policies za obavijesti
CREATE POLICY "Authenticated users can view active notifications"
  ON seup_obavijesti FOR SELECT
  TO authenticated
  USING (aktivan = true);

CREATE POLICY "Admin users can insert notifications"
  ON seup_obavijesti FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Admin users can update notifications"
  ON seup_obavijesti FOR UPDATE
  TO authenticated
  USING (auth.uid() IS NOT NULL)
  WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Admin users can delete notifications"
  ON seup_obavijesti FOR DELETE
  TO authenticated
  USING (auth.uid() IS NOT NULL);

-- Policies za pročitane obavijesti
CREATE POLICY "Users can view their own read notifications"
  ON seup_obavijesti_procitane FOR SELECT
  TO authenticated
  USING (fk_user = (auth.jwt() ->> 'sub')::integer);

CREATE POLICY "Users can mark notifications as read"
  ON seup_obavijesti_procitane FOR INSERT
  TO authenticated
  WITH CHECK (fk_user = (auth.jwt() ->> 'sub')::integer);

CREATE POLICY "Users can delete their own read marks"
  ON seup_obavijesti_procitane FOR DELETE
  TO authenticated
  USING (fk_user = (auth.jwt() ->> 'sub')::integer);

-- Test data
INSERT INTO seup_obavijesti (naslov, sadrzaj, tip, aktivan, fk_user_kreirao) VALUES 
